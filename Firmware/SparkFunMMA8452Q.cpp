...
byte MMA8452Q::begin(MMA8452Q_Scale fsr, MMA8452Q_ODR odr)
{
	...
	// SH: Read the interrupt source on start to ensure
	// we don't trash the contents of it during setup.
	interruptSource = readInterruptSource(); 
...
// SH: Enable tap interrupts.
void MMA8452Q::enableTapInterrupts() {
    // Set-up the acceleration sensor to interrupt on tap.
    // register 0x2D (Interrupt register, CTRL_REG4).
    // Bit 3 (0x08) INT_EN_PULSE
    // register 0x2E interrupt route.
    // Bit 3 (0x08) Set = INT1, clear = INT2
    // register 0x0C (Interrupt source)
    // Bit 3 (0x08) SRC PULSE
    
    standby();
    writeRegister(CTRL_REG4, 0x08);
    
    // Route through to INT1
    writeRegister(CTRL_REG5, 0x08);
    
    // SH:
    // 0b 0000 1011 // Pulse can wake, IPOL = actuve high, PP_OD open drain. -> 0x0B
    
    // Push-pull/open drain selection on interrupt pad. Default value: 0.
    // 0: Push-pull; 1: Open drain
    // 0x 0000 1010 -> 0x0A
    // writeRegister(CTRL_REG3, 0x0A); // previous setting. was push pull
    writeRegister(CTRL_REG3, 0x0B); // try open drain.
...
// NB: This uses the stored startup interrupt source value.
// Bit 7 : SRC_ASLP - Auto-sleep/wake interrupt status bit.
// Bit 6 : Always 0
// Bit 5 : SRC_TRANS - Transient interrupt status bit. D
// Bit 4 : SRC_LNDPRT - Landscape/portrait orientation interrupt status bit
// Bit 3 : SRC_PULSE - Pulse interrupt status bit
// Bit 2 : SRC_FF_MT - Freefall/motion interrupt status bit
// Bit 1 : Always 0
// Bit 0 : SRC_DRDY - Data-ready interrupt bit status
byte MMA8452Q::readInterruptSource() {
    // If the stored interrupt source is set then return that first time 
    // and then clear it to ensure we re-read the interrupt source
